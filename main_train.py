...
sites_PSR = transfer_input(args.pred_len, '1-', 0, 0, args.sites_trim, args.pars, args.batch_size_src, args.site_target, args.pred_flag, args.input_flag, args.PSR_params, 'PSR')
tc = joblib.load('./data/sites_train_classic')
print('tc loaded')
tl = joblib.load('./data/sites_train_label')
print('tl loaded')
vc = joblib.load('./data/sites_valid_classic')
print('vc loaded')
vl = joblib.load('./data/sites_valid_label')
print('vl loaded')
...
for pre in pres:
    for i in range(len(fixes)):
        for j in range(len(args.sites_trim)):
            print(pre+fixes[i]+'_'+args.sites_trim[j])
            args.site_target = args.sites_trim[j]
            PSR = [sites_PSR[pre][i][j][-1], sites_PSR['m']]
            exp = Exp_Informer(args, PSR)
            src_dl_l, tgt_dl_l, sites_PSR = transfer_input(1, pre, i, j, args.sites_trim, args.pars, args.batch_size_src, args.site_target, args.pred_flag, args.input_flag, args.PSR_params, 'train', tc, tl, vc, vl)
            src_dl, tgt_dl, sites_PSR = transfer_input(args.pred_len, pre, i, j, args.sites_trim, args.pars, args.batch_size_src, args.site_target, args.pred_flag, args.input_flag, args.PSR_params, 'train', tc, tl, vc, vl)
            if not os.path.isfile("./results/{}/v_{}/{}/model_{}_l".format(args.site_target, pre+fixes[i], '6', args.model)):
                losses, preds, labels = exp.train(pre, fixes[i], src_dl_l, tgt_dl, tgt_dl, 'src', i, 'lstm')
            if not os.path.isfile("./results/{}/v_{}/{}/model_{}".format(args.site_target, pre+fixes[i], args.pred_len, args.model)):
                losses, preds, labels = exp.train(pre, fixes[i], src_dl, tgt_dl, tgt_dl, 'src', i, 'model')
                losses_pre.append(losses)
                preds_pre.append(preds)
                labels_pre.append(labels)

joblib.dump(losses_pre, './results/log/losses_pre_{}'.format(args.pred_len))
joblib.dump(preds_pre, './results/log/preds_pre_{}'.format(args.pred_len))
joblib.dump(labels_pre, './results/log/labels_pre_{}'.format(args.pred_len))